      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#020617]">
        <header className="h-20 border-b border-slate-800 flex items-center justify-between px-8 bg-[#020617]/80 backdrop-blur-xl z-30">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {status === TaskStatus.RUNNING ? 'Остановить' : 'Запустить цикл'}
            </button>
          </div>
        </header>
                ...
                <div key={acc.id} className="bg-[#0b1120] border border-slate-800 rounded-[2.5rem] p-7 shadow-xl hover:border-blue-500/30 transition-all group overflow-hidden relative">
                  ...
                  <div className="pt-6 border-t border-slate-800/50 flex items-center justify-between">
                     ...
                     <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold text-emerald-500 uppercase italic">Protected</span>
                     </div>
                  </div>
                  <div className="absolute -right-6 -bottom-6 opacity-5 group-hover:scale-110 group-hover:opacity-10 transition-all duration-700 pointer-events-none">
                     <Database size={120} />
                  </div>
                </div>
              ))}
              ...
              <div className="bg-[#0b1120] border border-slate-800 rounded-[3rem] p-10 space-y-10 shadow-2xl relative overflow-hidden">
                 ...
                 <section className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <section>
                          <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-4">Стиль комментирования</label>
                          <select 
                            className="w-full bg-[#020617] border border-slate-800 rounded-2xl p-5 text-sm outline-none focus:ring-2 focus:ring-blue-500 text-white font-bold appearance-none shadow-inner"
                            value={settings.commentTemplate}
                            onChange={e => setSettings({...settings, commentTemplate: e.target.value})}
                          >
                             <option>Умный эксперт</option>
                             <option>Агрессивный маркетолог</option>
                             <option>Душа компании</option>
                             <option>Технарь-зануда</option>
                          </select>
                       </section>
                       <div className="grid grid-cols-2 gap-4">
                          <section>
  ...
  >
    <div className="flex items-start justify-between">
      <div className="flex gap-4">
        <div className={`p-3 rounded-2xl ${active ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-500 shadow-inner'}`}>
          <Zap size={20} />
        </div>
        <div>
          <h4 className="font-bold text-white text-lg tracking-tight">{name}</h4>
          <p className="text-[11px] text-slate-500 mt-1 max-w-md leading-relaxed">{description}</p>
        </div>
      </div>
      <div className={`flex items-center gap-2 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${active ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-900 text-slate-600 hover:text-slate-400'}`}>
...
const SidebarBtn: React.FC<{ active: boolean, onClick: () => void, icon: React.ReactNode, label: string, count?: number }> = ({ active, onClick, icon, label, count }) => (
  ...
  >
    {active && <div className="absolute left-0 top-1/4 bottom-1/4 w-1 bg-white rounded-r-full shadow-[0_0_10px_white]"></div>}
    ...
    {count !== undefined && <span className={`text-[9px] font-black px-2.5 py-1 rounded-lg ${active ? 'bg-white/20' : 'bg-slate-800 shadow-inner'}`}>{count}</span>}
  </button>
);

const StatCard: React.FC<{ title: string, value: number | string, icon: React.ReactNode }> = ({ title, value, icon }) => (
  <div className="bg-[#0b1120] border border-slate-800 p-6 rounded-[2.5rem] shadow-xl group hover:border-slate-700 transition-all relative overflow-hidden">
    <div className="relative z-10">
      <div className="text-3xl font-black text-white tracking-tighter mb-1 tabular-nums">{value}</div>
...
const LimitBar: React.FC<{ label: string, current: number, max: number, color: string }> = ({ label, current, max, color }) => (
  <div>
    <div className="flex justify-between text-[9px] font-black uppercase tracking-widest mb-2.5">
       ...
       <span className="text-slate-300 tabular-nums">{Math.round((current/max)*100)}%</span>
    </div>
    <div className="h-2 bg-slate-950 rounded-full overflow-hidden p-0.5 border border-slate-800 shadow-inner">
       <div 
         className={`h-full ${color} rounded-full transition-all duration-1000 shadow-[0_0_12px_rgba(59,130,246,0.2)]`} 
